...
export class ChartComponent implements OnChanges, AfterViewInit {
  ...
  createChart(openMode: string): void {
    ...
    const label: any = svg.append('g')
      ...
      .style('fill-opacity', (d: Node) => d.parent === focus || !d.children ? 1 : 0)
    ...
    function zoomTo(v) {
      ...
      node.attr('transform', (d: Node) => `translate(${(d.x - v[0]) * k},${(d.y - v[1]) * k})`);
      node.attr('r', (d: Node) => d.r * k);
    }
